class ComprasController < ApplicationController
  ...
  #compras/1/cuota
  def cuota
    @cuotas = PagoCompra.where("compra_id=?",params[:id])
    @tarjet = Tarjet.all
  end

  #compras/1/deta
  def deta
    @detalle = DetalleCompra.where("compra_id=?",params[:id])
  end
  ...
  private
    ...
    def compra_params
      params.require(:compra).permit(:com_cant, :com_total, :com_cuota, :cliente_id, :tipo_pago_id, :tipo_compra_id)
    end
end
